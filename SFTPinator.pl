...
$mw->g_wm_title("Two Point SFTPinator | File Selection");
...
$pw->g_wm_title('Two Point SFTPinator | Processing Files');
...
$lw->g_wm_title('Two Point SFTPinator | SFTP');
...
$mw->g_grid_columnconfigure(1, -weight => 0);
...
#	}
	...
	unless(open $gpg_log, "<", "gnupg/gpglog.txt"){
		$msg = "Failed to open gpglog.txt for reading.";
		$encrypted = 2;
		return;
		
	}
	chomp(my @gpg_log_read = <$gpg_log>);
	close $gpg_log;

	if($gpg_log_read[$#gpg_log_read] !~ /^gpg:\s+imported: 1$/){
		$msg = "Failed to import $key.";
		$encrypted = 2;
		return;
	}

...
my $hashPopulated:shared = undef;
my $parentDir:shared = undef;
my %fileHash;#:shared; #Hash for storing file information
...
$mw->g_wm_title("Two Point Example Uploader | File Selection");
...
$pw->g_wm_title('Two Point Example Uploader | Processing Files');
...
$lw->g_wm_title('Two Point Example Uploader | SFTP');
...
sub select_files {
	...
	if(defined($files)){
		$thr = threads->create(sub{ &thrPopFileHash($seleType, $files, $fileHashRef) });
		$thr->detach();
		&popHashChk($seleType, $fileHashRef, $tviewRef);
	}
}

sub popHashChk {
	my ($seleType, $fileHashRef, $tviewRef) = @_;
	if($hashPopulated){
		if($hashPopulated == 2){
			&onCleanExit(1, $msg);		
		}
		if ($seleType eq 'file'){
			...
			if(!($$tviewRef->exists($parentDir))){                   		
				$$tviewRef->insert("", "end", -id => $parentDir, -tags => "directory");
			}
			$$tviewRef->set($parentDir, "name", $$fileHashRef{$parentDir}->[0]);
			$$tviewRef->set($parentDir, "path", $parentDir);
			$$tviewRef->set($parentDir, "size", &scale_size($$fileHashRef{$parentDir}->[1]));
			$$tviewRef->set($parentDir, "date", $$fileHashRef{$parentDir}->[2]);
			find(\&add_dir_tree, "$parentDir");
			$$tviewRef->item($parentDir, -open => "true");
			#$parentDir = undef;
	foreach(keys %fileHash){
		print "Key: $_; Vals: $fileHash{$_}->[0], $fileHash{$_}->[1], $fileHash{$_}->[2]\n";
	}
		}
		...
		Tkx::after(500, [sub{ &popHashChk($seleType, $fileHashRef, $tviewRef) }]);
	}
}

sub thrPopFileHash {
	my ($seleType, $files, $fileHashRef) = @_;
	my @fStats = undef;
	my %dups;
	...
	if($printDups eq 1){
		...
		$msg = "Multiple files found with the same filename, please re-name the files listed in '$initialDir\\TwoPoint_duplicates.txt' and start again.";
		$hashPopulated = 2;
		return;
	}
...
sub add_dir_tree {
	if(-f $_){
		if(!($tview->exists($File::Find::name))){
